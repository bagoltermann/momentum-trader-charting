...
| **Ollama** | Latest | https://ollama.ai (for LLM validation, optional) |
...
### "Ollama not available" in LLM Validation panel
Ollama is not running. The launcher .bat starts it automatically, or run manually:
```powershell
ollama serve
```
Make sure you've pulled the model (from momentum-trader setup):
```powershell
...
## LLM Validation Feature

The charting app includes an LLM-powered validation feature that analyzes stock setups using Warrior Trading methodology.

### How it works:
- **Top 3 Panel**: Auto-validates top 3 candidates (5 Pillars score ≥3.5) every 60 seconds
- **Manual Validate**: Click "Validate" button in header to analyze selected chart
- **Signals**: Returns BUY/WAIT/NO_TRADE with entry, stop, target prices and reasoning

### Requirements:
- Ollama must be running (launcher starts it automatically)
- Model `qwen2.5:7b` must be pulled (handled by momentum-trader setup)

### Configuration:
LLM settings are in `config/charting.yaml` (auto-generated by launcher):
```yaml
llm:
  enabled: true
  ollama:
